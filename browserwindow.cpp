...
BrowserWindow::BrowserWindow(Browser *browser, QWebEngineProfile *profile, bool forDevTools, bool isMain)
    : m_browser(browser)
    , m_profile(profile)
    , m_tabWidget(new TabWidget(profile, this))
{
    ...
    if (!isMain) {
        ...
        UrlSchemeHandler* urlHandler = new UrlSchemeHandler();
        m_profile->setUrlRequestInterceptor(interceptor);
        m_profile->installUrlSchemeHandler(urlHandler->scheme, urlHandler);
    ...
    if (!forDevTools) { // && !isMain) {
        ...
        if (!isMain) {
            addToolBar(toolbar);
            QStatusBar* statusbar = createStatusBar();
    ...
    if (!forDevTools && isMain) {
        connect(m_tabWidget, &TabWidget::onViewArchiveRequested, this, &BrowserWindow::onViewArchiveRequestedKv);
    }
...
QMenu *BrowserWindow::createViewMenu(QToolBar *toolbar)
{
    ...
    // API reference - Class Qt::Key
    // https://www.klayout.de/doc/code/class_Qt::Key.html
    ...
    QAction *zoomReset = viewMenu->addAction(tr("Zoom &Reset"));
    ...
    connect(viewSettingsAction, &QAction::triggered, this, &BrowserWindow::handleCreateSettingsWindow );
...
QToolBar *BrowserWindow::createToolBar()
{
    ...
    m_historyBackAction->setIcon(QIcon(u":/icons/go_back.png"_s));
    ...
    m_historyForwardAction->setIcon(QIcon(u":/icons/go_next.png"_s));
    ...
    **/
    ...
    QAction* pdfAction = new QAction(this);
    pdfAction->setIcon(QIcon(u":/icons/pdf.png"_s));
    navigationBar->addAction(pdfAction);
    connect(pdfAction, &QAction::triggered, [=](){
        WebPageExporter* exporter = new WebPageExporter(this);
        exporter->exportToPdf(currentTab()->page());
    });

    QAction* pngAction = new QAction(this);
    pngAction->setIcon(QIcon(u":/icons/png.png"_s));
    navigationBar->addAction(pngAction);
    connect(pngAction, &QAction::triggered, [=]() {
        WebPageExporter* exporter = new WebPageExporter(this);
        exporter->exportToPng(currentTab()->page());
    });

    return navigationBar;
}

QStatusBar *BrowserWindow::createStatusBar()
{
    QStatusBar* bar = new QStatusBar();
    bar->setLayout(new QHBoxLayout());
    QLabel* idIconLabel = new QLabel("<html><img src=':/icons/lock.png'></html>");
    _labelKVId = new QLabel("");
    QLabel* siteIconLabel = new QLabel("<html><img src=':/icons/file.png'></html>");
    _labelKVsiteName = new QLabel("");
    QLabel* dateIconLabel = new QLabel("<html><img src=':/icons/cal.png'></html>");
    _labelKVDate = new QLabel("");
...
void BrowserWindow::onViewArchiveRequestedKv(const QString &url, const QString &archiveId, const QString &jsonText)
{
    emit onViewArchiveRequested(url, archiveId, jsonText);
}
...
void BrowserWindow::setKvParams(const QString &url, const QString &id, const QString &json)
{
    _kvUrl = url;
    _kvId = id;
    _kvJson = json;
    _labelKVsiteName->setText(id);
    _labelKVId->setText(url);
...
void BrowserWindow::handleWebViewLoadProgress(int progress)
{
    ...
    static QIcon reloadIcon(u":/icons/refresh.png"_s);
...
void BrowserWindow::handleCreateSettingsWindow()
{
    ...
    tab->addTab(serverW, "Server Settings");
    tab->addTab(contentW, "Content Settings");
    dlg->show();
